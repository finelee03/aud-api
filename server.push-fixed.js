...
// [ADD] normalize PushSubscription from browser (FCM/standard)
function normalizeSubscription(raw){
  try{
    ...
    if (!p256dh || !auth) return { ok:false, error:'missing_keys' };
    return {
      ok: true,
...
app.post('/api/push/subscribe', express.json(), (req, res) => {
  ...
  try {
    ...
    upsertSubscriptionRow(ns, norm);
    return res.json({ ok:true, ns: String(ns||'').toLowerCase() });
  } catch (e) {
